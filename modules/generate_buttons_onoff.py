...
for fn in range(1, 101, 1):  # function number
    ...
    file.write(space + f"if globals.enabled[{fn}-1]:" + enter)
    file.write(space + space + f"globals.enabled[{fn}-1] = 0" + enter)
    file.write(space + space + f"buttons.turnToGrayImages(TABs, {fn})" + enter)
    file.write(space + f"else:" + enter)
    file.write(space + space + f"globals.enabled[{fn}-1] = 1" + enter)
